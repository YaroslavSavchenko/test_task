...
function stepfinal() {
    ...
    var emptyBox = document.querySelectorAll(".abierta:not(.premiazo)");
    var prizeBox = document.querySelector(".premiazo").getAttribute("id");
    ...
    if (emptyBox) {
        for (let i = 0; i < emptyBox.length; i++) {
            var boxId = emptyBox[i].getAttribute("id"); 
            emptyBoxText += `Коробка под id = ${boxId} была пустой при открытии \n`;
        }
    }
    
    if (prizeBox) {
        prizeBoxText += `Коробка под id = ${prizeBox} была с призом телефона при открытии`;
    }
    
    console.log(emptyBoxText);
    console.log(prizeBoxText);
}
